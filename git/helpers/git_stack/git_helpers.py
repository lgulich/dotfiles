...
def _run_git(repo_path: Path, args: list[str], check: bool = True, input_text: str | None = None) -> str:
    """
...
def create_commit(repo_path: Path, filename: str, message: str, content: str | None = None) -> str:
    """
...
def amend_commit(repo_path: Path, filename: str, commit_back: int = 0, content: str | None = None) -> str:
    """
    ...
    try:
        current_branch = _run_git(repo_path, ['symbolic-ref', '--short', 'HEAD'], check=False)
    ...
    if commit_back > 0:
        ...
        later_commits = _run_git(
            repo_path, 
            ['rev-list', '--reverse', f'{target_sha}..HEAD']
        ).split('\n')
        if later_commits == ['']:
            later_commits = []
    else:
    ...
    parent_sha = _run_git(repo_path, ['rev-parse', f'{target_sha}~1'], check=False)
...
def get_current_branch(repo_path: Path) -> str | None:
    ...
    """
    try:
        branch = _run_git(repo_path, ['symbolic-ref', '--short', 'HEAD'], check=False)
        return branch if branch else None
    except:
        return None


def create_remote(repo_path: Path, remote_name: str, remote_path: Path) -> None:
    """
...
def push_branch(repo_path: Path, remote_name: str, branch_name: str, force: bool = False) -> None:
    """
