#!/usr/bin/env python3
# pylint: disable=too-many-lines,too-many-instance-attributes,too-many-branches,too-many-locals
"""
...
from git_helpers import (create_git_repo, create_commit, amend_commit,
                         create_branch, checkout, get_commit_message,
                         get_current_branch, branch_exists, create_remote)
...
class GitStackTestCase(unittest.TestCase):
    ...
    def create_stack_instance(self, dry_run=False, stack_name=None):
        ...
        """
        return GitStackPush(dry_run=dry_run,
    ...
    def get_change_id(self, repo_path: Path, ref: str = 'HEAD') -> str:
        ...
        change_id = extract_change_id(msg)
        assert change_id is not None, f"No Change-ID found in commit {ref}"
        return change_id
    ...
    def check_operations(self,
        ...
        if order_matters:
            ...
            for i, (actual,
                    expected) in enumerate(zip(operations,
                                               expected_operations)):
            ...
            for exp_idx, expected in enumerate(expected_operations):
                ...
                if not found_match:
                    self.fail(
                        f"Could not find match for expected operation {exp_idx}: "
                        f"{expected['operation']} with args {expected['args']}"
                    )

    def _match_operation(self,
                         actual: dict,
                         expected: dict,
                         index: int,
        ...
        if actual['operation'] != expected['operation']:
            if raise_on_mismatch:
                self.assertEqual(
                    ...
                    f"Operation {index}: expected "
                    f"'{expected['operation']}', "
                    f"got '{actual['operation']}'")
            else:
        ...
        if set(actual['args'].keys()) != set(expected['args'].keys()):
            if raise_on_mismatch:
                self.assertEqual(set(actual['args'].keys()),
                                 set(expected['args'].keys()),
        ...
        for key, expected_value in expected['args'].items():
            ...
            if isinstance(expected_value, str):
                ...
                if not re.fullmatch(expected_value, str(actual_value),
                                    re.DOTALL):
                    if raise_on_mismatch:
                        self.assertIsNotNone(
                            re.fullmatch(expected_value, str(actual_value),
                                         re.DOTALL),
                            f"Operation {index}: arg '{key}' value "
                            f"'{actual_value}' does not match pattern "
                            f"'{expected_value}'")
                    else:
...
class TestPushBasicStack(GitStackTestCase):
    ...
    def test_push_basic_stack(self):
        ...
        # Create 3 commits
        _sha1 = create_commit(self.repo_path, 'file1.txt', 'First commit')
        _sha2 = create_commit(self.repo_path, 'file2.txt', 'Second commit')
        _sha3 = create_commit(self.repo_path, 'file3.txt', 'Third commit')
...
class TestReindex(GitStackTestCase):
    ...
    def test_reindex_creates_new_ids(self):
        ...
        # Get MR IIDs
        mapping_before = self.read_mapping()
        _old_mr_iids = {info['mr_iid'] for info in mapping_before.values()}
